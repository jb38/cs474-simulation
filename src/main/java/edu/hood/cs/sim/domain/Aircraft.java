...
import edu.hood.cs.sim.Instrumentation;
...
public class Aircraft implements Steppable {
	...
	public void init(SimState state) {
		...
		ScheduledFlightRepository repository = ScheduledFlightRepository
				.getInstance();
		this.schedule = repository.getSchedule(this);

		log.trace("schedule for " + this.tailNum + " has "
				+ this.schedule.size() + " items");

		if (this.schedule.size() == 0) {
			return;
		} else {
			sim.schedule.scheduleOnce(this.schedule.get(0)
	...
	public void step(SimState state) {
		...
		double departureTime = currentTask.getDepartureSimTime()
				+ this.getDelay();
		...
		// should land?
			...
			if (this.schedule.size() > 0) {
				...
				if (time < nextTask.getDepartureSimTime()) {
					sim.schedule.scheduleOnce(nextTask.getDepartureSimTime(),
							this);
				} else {
					log.debug(String.format("%8.1f %s erroneous", time,
							nextTask));
				}
		...
		// TODO calculate the aircraft's geographic position? doing this depends
		// on
		// how we decide to implement the visualization

		// double progress = Math.min(0, (steps - departureSteps) /
		// (double)(arrivalSteps - departureSteps)),
		// x = 0,
